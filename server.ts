...
function getMakeWASocket() {
    if (typeof (BaileysNamespace as any).default === 'function') return (BaileysNamespace as any).default;
    if (typeof (BaileysNamespace as any).makeWASocket === 'function') return (BaileysNamespace as any).makeWASocket;
    if (BaileysNamespace && typeof (BaileysNamespace as any).default?.default === 'function') return (BaileysNamespace as any).default.default;
    // Fallback search
    const keys = Object.keys(BaileysNamespace);
    for (const key of keys) {
        if (typeof (BaileysNamespace as any)[key] === 'function' && key.toLowerCase().includes('socket')) {
            return (BaileysNamespace as any)[key];
        }
    }
    return (BaileysNamespace as any).default || BaileysNamespace;
}
...
// MongoDB Connection
const mongoURI = process.env.MONGODB_URI || "mongodb+srv://Bebas:Bebas123@cluster0.jmthq2c.mongodb.net/?appName=Cluster0";
