...
if (apiKey) {
    // Dynamic import to prevent crash if module is missing
    import("@google/generative-ai")
        .then(({ GoogleGenerativeAI }) => {
            const genAI = new GoogleGenerativeAI(apiKey);
            model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });
            console.log("Gemini AI initialized (v3).");
        })
        .catch(e => {
            console.error("Gemini AI dynamic import failed:", e);
            console.log("Tip: Ensure @google/generative-ai is installed in the deployment environment.");
        });
} else {
